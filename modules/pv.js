...
const ACHIEVEMENTS = [
  ...
  {id:'perfect-world', title:'Perfect World', icon:'🌟', desc:'Complete every lesson in a unit.'},
  ...
  {id:'wide-reader', title:'Wide Reader', icon:'🧠', desc:'Earn 1000 XP.'},
  {id:'explorer-badge', title:'Explorer', icon:'🗺️', desc:'Visit lessons across 5 units.'},
  {id:'poorvi-guardian', title:'Guardian of Poorvi', icon:'🌅', desc:'Defeat the final challenge of Poorvi.'},
];
const AVATARS = [{id:'explorer',emoji:'📖'},{id:'scribe',emoji:'✍️'},{id:'sage',emoji:'🖋️'},{id:'astro',emoji:'🎭'}];
...
const key = (w,l)=>`${w}:${l}`;
function activeUnreviewedWorldPv(){
  for(const w of CU){
    const started = w.lessons.some(l=>!!STATE.completed[key(w.id,l.id)]);
    const reviewed = typeof atlasModulePassed !== 'function' || atlasModulePassed('pv', String(w.id));
    if(started && !reviewed) return w;
  }
  return null;
}
function isWorldUnlocked(worldId){
  const active = activeUnreviewedWorldPv();
  return !active || active.id===worldId;
}
function worldNeedsReviewPv(worldId){
  const active = activeUnreviewedWorldPv();
  return !!active && active.id!==worldId;
}
window.__openWorldReviewPv = ()=>{
  const active = activeUnreviewedWorldPv();
...
function completeNode(e){
  ...
  if(xp>=1000) grantBadge('wide-reader');
  ...
  if(e.worldId===CU[CU.length-1]?.id && e.lessonId===CU[CU.length-1]?.lessons[CU[CU.length-1].lessons.length-1]?.id) grantBadge('poorvi-guardian');
...
if(window.__pvHashListener) window.removeEventListener('hashchange', window.__pvHashListener);
window.__pvHashListener = function(){ if(!location.hash.startsWith('#/pv')) return; render(); };
window.addEventListener('hashchange', window.__pvHashListener);
function render(){
  window.scrollTo(0,0);
  const hash = location.hash.replace(/^#\/pv\/?/,'');
...
function Topbar(){
  ...
  return `<div class="topbar">
    ...
    <a class="stat-chip" href="#/pv/achievements">🏅 ${STATE.badges.length}/${ACHIEVEMENTS.length}</a>
    <a class="stat-chip" href="#/pv/shop">🛍 Shop</a>
  </div>`;
}
...
function renderHome(){
  ...
  const body = `
    ...
    <div class="card" style="margin-top:14px">
      ...
      <div style="font-size:12px;color:var(--muted);margin-top:6px">missions recovered${curTarget?` · Current unit: ${curTarget.w.emoji} ${curTarget.w.name}`:''}</div>
    </div>
    ...
    <div>
      ${CU.map(w=>{
        ...
        const needsReview = worldNeedsReviewPv(w.id);
        const prog = worldProgress(w.id);
        const clickAttr = needsReview ? `onclick='__openWorldReviewPv()'` : '';
        return `<div class="world-card ${unlocked?'':(needsReview?'needs-review':'locked')}" ${clickAttr} style="background:linear-gradient(135deg,${w.palette.from},${w.palette.to});${needsReview?'cursor:pointer':''}">
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <div style="flex:1">
              <div style="font-size:10.5px;text-transform:uppercase;letter-spacing:.06em;opacity:.85">Unit ${w.index}</div>
              <div class="font-display" style="font-size:19px;font-weight:800">${w.name}</div>
              <div style="font-size:12.5px;opacity:.9">${needsReview?'Tap to take the Module Review →':(unlocked?w.subtitle:'Complete the previous unit to reveal this one')}</div>
            </div>
          ...
          ${unlocked?`<div class="node-grid">${w.lessons.map(l=>{
            ...
            return `<a class="node ${nodeUnlocked?'':'locked'} ${done?'done':''}" href="${nodeUnlocked?'#/pv/lesson/'+w.id+'/'+l.id:'#'}">
...
function renderAchievements(){
  const body = `<a href="#/pv/" style="font-size:13px;color:var(--muted)">← Home</a>
...
function renderLesson(worldId, lessonId){
  ...
  if(!w||!lesson){ location.hash='#/pv/'; return; }
    ...
    return `<div style="border-radius:32px;padding:2px;background:${grad}">
      <div style="border-radius:30px;background:#0d0818cc;backdrop-filter:blur(14px);padding:20px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
          <a href="#/pv/" style="border:2px solid var(--border);border-radius:12px;padding:6px 12px;font-weight:800">✕</a>
    ...
    if(stage==='done'){
      ...
      app.innerHTML = `<main>
        <div class="card" style="text-align:center;background:${grad};color:#fff">
          ...
          <div style="margin-top:20px"><a class="btn" style="background:#fff;color:#1a1030" href="#/pv/">Back to Progress Map</a></div>
        </div>
      </main>`;
      return;
    }
  ...
  function questionBody(q){
    ...
    if(q.kind==='passagetap'){
      return `<div class="passage-box">${q.segments.map((s,i)=>{
        ...
        let cls='passage-word'; if(st.picked!==undefined){ if(s.correct) cls+=' correct'; else if(i===st.picked) cls+=' wrong'; }
        return `<button class="${cls}" ${st.picked!==undefined?'disabled':''} onclick="__ansPassage(${i})">${s.text}</button>`;
      }).join('')}</div>`;
    }
    if(q.kind==='order'){
  ...
  function resolve(correct){
    ...
    if(correct){ correctCount++; combo++; bestCombo=Math.max(bestCombo,combo); }
      ...
      const q = questions[qIdx];
      if(typeof atlasRecordMistake === 'function') atlasRecordMistake('pv', w.name||'general', q ? {prompt:q.prompt, type:q.type} : null);
    }
    draw();
  }
  window.__ansPick = (i)=>{ const st=qState[qIdx]; if(st.picked!==undefined) return; st.picked=i; resolve(i===questions[qIdx].answer); };
  window.__ansPassage = (i)=>{ const st=qState[qIdx]; if(st.picked!==undefined) return; st.picked=i; resolve(!!questions[qIdx].segments[i].correct); };
...
window.SubjectApps['pv'] = { init: pvInit };
