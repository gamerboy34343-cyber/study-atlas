...
const hiClassId = localStorage.getItem('atlas-class') || '8';
const CU = (hiClassId!=='8' && window['HINDI_CURRICULUM_'+hiClassId]) ? window['HINDI_CURRICULUM_'+hiClassId] : window.HINDI_CURRICULUM;
const ACHIEVEMENTS = [
  ...
  {id:'kavi-hriday', title:'Kavi Hriday', icon:'🧠', desc:'Earn 1000 XP.'},
  {id:'explorer-badge', title:'Explorer', icon:'🗺️', desc:'Visit lessons across 5 worlds.'},
  {id:'malhar-guardian', title:'Guardian of Malhar', icon:'🦚', desc:'Defeat the final challenge of Malhar.'},
];
const AVATARS = [{id:'explorer',emoji:'📖'},{id:'scribe',emoji:'✍️'},{id:'sage',emoji:'🖋️'},{id:'astro',emoji:'🎭'}];
const PETS = [{id:'owl',emoji:'🦚'},{id:'fox',emoji:'🐦'},{id:'cat',emoji:'🕊️'},{id:'dragon',emoji:'🦜'}];

/* ============================== STATE ============================== */
const STORE_KEY = 'malhar-save-v1' + (hiClassId!=='8' ? '-c'+hiClassId : '');
...
function completeNode(e){
  ...
  if(xp>=1000) grantBadge('kavi-hriday');
  ...
  if(e.worldId==='tarun' && e.lessonId==='ts-boss') grantBadge('malhar-guardian');
...
if(window.__hiHashListener) window.removeEventListener('hashchange', window.__hiHashListener);
window.__hiHashListener = function(){ if(!location.hash.startsWith('#/hi')) return; render(); };
window.addEventListener('hashchange', window.__hiHashListener);
function render(){
  window.scrollTo(0,0);
  const hash = location.hash.replace(/^#\/hi\/?/,'');
...
function Topbar(){
  ...
  return `<div class="topbar">
    ...
    <a class="stat-chip" href="#/hi/achievements">🏅 ${STATE.badges.length}/${ACHIEVEMENTS.length}</a>
    <a class="stat-chip" href="#/hi/shop">🛍 Shop</a>
  </div>`;
}
...
function renderHome(){
  ...
  const body = `
    <div class="card" style="text-align:center">
      ...
      <h1 class="font-display" style="font-size:30px;margin:8px 0 4px">${hiClassId==='5'?'वीणा':'मल्हार'}</h1>
      <p style="color:var(--muted);max-width:520px;margin:0 auto">${hiClassId==='5'?'कविता, कहानी और पत्र — बारह पाठों की इस यात्रा में हर भाव, हर पंक्ति को गहराई से महसूस करें।':'कविता, कहानी, पत्र और उद्बोधन — दस अध्यायों की इस साहित्यिक यात्रा में हर भाव, हर पंक्ति को गहराई से महसूस करें।'}</p>
      <div style="margin-top:18px">
        ${curTarget?`<a class="btn btn-primary" href="#/hi/lesson/${curTarget.w.id}/${curTarget.l.id}">${doneLessons?'Continue':'Start'} the adventure →</a>`:`<div class="btn btn-primary" style="display:inline-block">🏆 All missions recovered!</div>`}
      </div>
    </div>
      ...
      ${CU.map(w=>{
        ...
        return `<div class="world-card ${unlocked?'':(needsReview?'needs-review':'locked')}" ${clickAttr} style="background:linear-gradient(135deg,${w.palette.from},${w.palette.to});${needsReview?'cursor:pointer':''}">
          ...
          ${unlocked?`<div class="node-grid">${w.lessons.map(l=>{
            ...
            return `<a class="node ${nodeUnlocked?'':'locked'} ${done?'done':''}" href="${nodeUnlocked?'#/hi/lesson/'+w.id+'/'+l.id:'#'}">
...
function renderAchievements(){
  const body = `<a href="#/hi/" style="font-size:13px;color:var(--muted)">← Home</a>
...
function renderLesson(worldId, lessonId){
  ...
  if(!w||!lesson){ location.hash='#/hi/'; return; }
    ...
    return `<div style="border-radius:32px;padding:2px;background:${grad}">
      <div style="border-radius:30px;background:#0d0818cc;backdrop-filter:blur(14px);padding:20px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
          <a href="#/hi/" style="border:2px solid var(--border);border-radius:12px;padding:6px 12px;font-weight:800">✕</a>
    ...
    if(stage==='done'){
      ...
      app.innerHTML = `<main>
        <div class="card" style="text-align:center;background:${grad};color:#fff">
          ...
          <div style="margin-top:20px"><a class="btn" style="background:#fff;color:#1a1030" href="#/hi/">Back to Progress Map</a></div>
        </div>
      </main>`;
      return;
    }
  ...
  function resolve(correct){
    ...
    if(correct){ correctCount++; combo++; bestCombo=Math.max(bestCombo,combo); }
      ...
      const q = questions[qIdx];
      if(typeof atlasRecordMistake === 'function') atlasRecordMistake('hi', w.name||'general', q ? {prompt:q.prompt, type:q.type} : null);
    }
    draw();
  }
...
window.SubjectApps['hi'] = { init: hiInit };
