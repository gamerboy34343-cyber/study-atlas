...
function worldReviewPassed(i){
  if(i===0) return true;
  return typeof atlasModulePassed !== 'function' || atlasModulePassed('cv', WORLDS[i-1].id);
}
function worldAlreadyStarted(i){
  const w = WORLDS[i];
  return w.levels.filter(l=>(STATE.progress[progKey(w.id,l.id)]?.stars??0)>0).length > 0;
}
function isWorldUnlockedCv(i){ return i===0 || worldAlreadyStarted(i) || (worldPrevDone(i) && worldReviewPassed(i)); }
function worldNeedsReviewCv(i){ return i>0 && !worldAlreadyStarted(i) && worldPrevDone(i) && !worldReviewPassed(i); }
window.__openWorldReviewCv = (i)=>{
  ...
  app.innerHTML = `${Nav()}<main>
    ...
    <div style="display:grid;gap:14px">
      ${WORLDS.map((w,i)=>{
        ...
        const clickAttr = needsReview ? `onclick="__openWorldReviewCv(${i});return false;"` : '';
