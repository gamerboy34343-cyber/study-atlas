  ...
  app.innerHTML = `${Nav()}<main>
    ...
    <div style="display:grid;gap:14px">
      ${WORLDS.map((w,i)=>{
        ...
        const href = unlocked ? '#/world/'+w.slug : '#';
...
function renderWorld(slug){
  ...
  if(!isWorldUnlockedGq(wIdx)){ render(); return; }
  app.innerHTML = `${Nav()}<main>
    ...
    <div style="margin-top:20px;display:grid;gap:12px">
      ${w.levels.map((lv,i)=>{
        ...
        return `<a class="lesson-row glass-strong ${unlocked?'':'locked'}" href="${unlocked?'#/lesson/'+w.id+'/'+lv.id:'#'}">
