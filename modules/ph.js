...
const phClassId = localStorage.getItem('atlas-class') || '8';
const CU_EN = (phClassId!=='8' && window['PHYSICS_CURRICULUM_'+phClassId]) ? window['PHYSICS_CURRICULUM_'+phClassId] : window.PHYSICS_CURRICULUM;
const CU_HI = (phClassId!=='8' && window['PHYSICS_CURRICULUM_'+phClassId+'_HI']) ? window['PHYSICS_CURRICULUM_'+phClassId+'_HI'] : window.PHYSICS_CURRICULUM_HI;
...
const ACHIEVEMENTS = [
  ...
  {id:'physics-genius', title:'Physics Genius', icon:'🧠', desc:'Earn 1000 XP.'},
  {id:'explorer-badge', title:'Explorer', icon:'🗺️', desc:'Visit lessons across 5 worlds.'},
  {id:'physics-champion', title:'Physics Champion', icon:'🏆', desc:'Defeat the final boss.'},
];
...
/* ============================== STATE ============================== */
const STORE_KEY = 'physics-quest-save-v1' + (phClassId!=='8' ? '-c'+phClassId : '');
...
const key = (w,l)=>`${w}:${l}`;
function activeUnreviewedWorldPh(){
  for(const w of CU){
    const started = w.lessons.some(l=>!!STATE.completed[key(w.id,l.id)]);
    const reviewed = typeof atlasModulePassed !== 'function' || atlasModulePassed('ph', String(w.id));
    if(started && !reviewed) return w;
  }
  return null;
}
function isWorldUnlocked(worldId){
  const active = activeUnreviewedWorldPh();
  return !active || active.id===worldId;
}
function worldNeedsReviewPh(worldId){
  const active = activeUnreviewedWorldPh();
  return !!active && active.id!==worldId;
}
window.__openWorldReviewPh = ()=>{
  const active = activeUnreviewedWorldPh();
...
function completeNode(e){
  ...
  if(xp>=1000) grantBadge('physics-genius');
  ...
  if(e.worldId==='waves11' && e.lessonId==='wv-boss') grantBadge('physics-champion');
...
if(window.__phHashListener) window.removeEventListener('hashchange', window.__phHashListener);
window.__phHashListener = function(){ if(!location.hash.startsWith('#/ph')) return; render(); };
window.addEventListener('hashchange', window.__phHashListener);
function render(){
  window.scrollTo(0,0);
  const hash = location.hash.replace(/^#\/ph\/?/,'');
...
function Topbar(){
  ...
  return `<div class="topbar">
    ...
    <a class="stat-chip" href="#/ph/achievements">🏅 ${STATE.badges.length}/${ACHIEVEMENTS.length}</a>
    <a class="stat-chip" href="#/ph/shop">🛍 Shop</a>
  </div>`;
}
...
function renderHome(){
  ...
  const body = `
    <div class="card" style="text-align:center">
      ...
      <h1 class="font-display" style="font-size:30px;margin:8px 0 4px">Physics Odyssey</h1>
      <p style="color:var(--muted);max-width:520px;margin:0 auto">From falling apples to travelling waves — master the forces, motion, and energy that govern the physical universe, one law at a time.</p>
      <div style="margin-top:18px">
        ${curTarget?`<a class="btn btn-primary" href="#/ph/lesson/${curTarget.w.id}/${curTarget.l.id}">${doneLessons?'Continue':'Start'} the adventure →</a>`:`<div class="btn btn-primary" style="display:inline-block">🏆 All missions recovered!</div>`}
      </div>
    </div>
    ...
    <div>
      ${CU.map(w=>{
        ...
        const needsReview = worldNeedsReviewPh(w.id);
        const prog = worldProgress(w.id);
        const clickAttr = needsReview ? `onclick='__openWorldReviewPh()'` : '';
        return `<div class="world-card ${unlocked?'':(needsReview?'needs-review':'locked')}" ${clickAttr} style="background:linear-gradient(135deg,${w.palette.from},${w.palette.to});${needsReview?'cursor:pointer':''}">
          ...
          ${unlocked?`<div class="node-grid">${w.lessons.map(l=>{
            ...
            return `<a class="node ${nodeUnlocked?'':'locked'} ${done?'done':''}" href="${nodeUnlocked?'#/ph/lesson/'+w.id+'/'+l.id:'#'}">
...
function renderAchievements(){
  const body = `<a href="#/ph/" style="font-size:13px;color:var(--muted)">← Home</a>
...
function renderLesson(worldId, lessonId){
  ...
  if(!w||!lesson){ location.hash='#/ph/'; return; }
    ...
    return `<div style="border-radius:32px;padding:2px;background:${grad}">
      <div style="border-radius:30px;background:#0d0818cc;backdrop-filter:blur(14px);padding:20px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
          <a href="#/ph/" style="border:2px solid var(--border);border-radius:12px;padding:6px 12px;font-weight:800">✕</a>
    ...
    if(stage==='done'){
      ...
      app.innerHTML = `<main>
        <div class="card" style="text-align:center;background:${grad};color:#fff">
          ...
          <div style="margin-top:20px"><a class="btn" style="background:#fff;color:#1a1030" href="#/ph/">Back to Progress Map</a></div>
        </div>
      </main>`;
      return;
    }
  ...
  function resolve(correct){
    ...
    if(correct){ correctCount++; combo++; bestCombo=Math.max(bestCombo,combo); }
      ...
      const q = questions[qIdx];
      if(typeof atlasRecordMistake === 'function') atlasRecordMistake('ph', w.name||'general', q ? {prompt:q.prompt, type:q.type} : null);
    }
    draw();
  }
...
window.SubjectApps['ph'] = { init: phInit };
