...
const chClassId = localStorage.getItem('atlas-class') || '8';
const CU_EN = (chClassId!=='8' && window['CHEMISTRY_CURRICULUM_'+chClassId]) ? window['CHEMISTRY_CURRICULUM_'+chClassId] : window.CHEMISTRY_CURRICULUM;
const CU_HI = (chClassId!=='8' && window['CHEMISTRY_CURRICULUM_'+chClassId+'_HI']) ? window['CHEMISTRY_CURRICULUM_'+chClassId+'_HI'] : window.CHEMISTRY_CURRICULUM_HI;
...
const ACHIEVEMENTS = [
  ...
  {id:'chemistry-genius', title:'Chemistry Genius', icon:'🧠', desc:'Earn 1000 XP.'},
  {id:'explorer-badge', title:'Explorer', icon:'🗺️', desc:'Visit lessons across 5 worlds.'},
  {id:'chemistry-champion', title:'Chemistry Champion', icon:'🏆', desc:'Defeat the final boss.'},
];
...
/* ============================== STATE ============================== */
const STORE_KEY = 'chemistry-quest-save-v1' + (chClassId!=='8' ? '-c'+chClassId : '');
...
const key = (w,l)=>`${w}:${l}`;
function activeUnreviewedWorldCh(){
  for(const w of CU){
    const started = w.lessons.some(l=>!!STATE.completed[key(w.id,l.id)]);
    const reviewed = typeof atlasModulePassed !== 'function' || atlasModulePassed('ch', String(w.id));
    if(started && !reviewed) return w;
  }
  return null;
}
function isWorldUnlocked(worldId){
  const active = activeUnreviewedWorldCh();
  return !active || active.id===worldId;
}
function worldNeedsReviewCh(worldId){
  const active = activeUnreviewedWorldCh();
  return !!active && active.id!==worldId;
}
window.__openWorldReviewCh = ()=>{
  const active = activeUnreviewedWorldCh();
...
function completeNode(e){
  ...
  if(xp>=1000) grantBadge('chemistry-genius');
  ...
  if(e.worldId==='hydrocarbons11' && e.lessonId==='hc-boss') grantBadge('chemistry-champion');
...
if(window.__chHashListener) window.removeEventListener('hashchange', window.__chHashListener);
window.__chHashListener = function(){ if(!location.hash.startsWith('#/ch')) return; render(); };
window.addEventListener('hashchange', window.__chHashListener);
function render(){
  window.scrollTo(0,0);
...
function Topbar(){
  ...
  return `<div class="topbar">
    ...
    <a class="stat-chip" href="#/ch/achievements">🏅 ${STATE.badges.length}/${ACHIEVEMENTS.length}</a>
    <a class="stat-chip" href="#/ch/shop">🛍 Shop</a>
  </div>`;
}
...
function renderHome(){
  ...
  const body = `
    <div class="card" style="text-align:center">
      ...
      <h1 class="font-display" style="font-size:30px;margin:8px 0 4px">Elemental Forge</h1>
      <p style="color:var(--muted);max-width:520px;margin:0 auto">From atoms to hydrocarbons — explore the hidden architecture of matter, master the periodic table, and forge your way through reactions real chemists study.</p>
      <div style="margin-top:18px">
        ${curTarget?`<a class="btn btn-primary" href="#/ch/lesson/${curTarget.w.id}/${curTarget.l.id}">${doneLessons?'Continue':'Start'} the adventure →</a>`:`<div class="btn btn-primary" style="display:inline-block">🏆 All missions recovered!</div>`}
      </div>
    </div>
    ...
    <div>
      ${CU.map(w=>{
        ...
        const needsReview = worldNeedsReviewCh(w.id);
        const prog = worldProgress(w.id);
        const clickAttr = needsReview ? `onclick='__openWorldReviewCh()'` : '';
        return `<div class="world-card ${unlocked?'':(needsReview?'needs-review':'locked')}" ${clickAttr} style="background:linear-gradient(135deg,${w.palette.from},${w.palette.to});${needsReview?'cursor:pointer':''}">
          ...
          ${unlocked?`<div class="node-grid">${w.lessons.map(l=>{
            ...
            return `<a class="node ${nodeUnlocked?'':'locked'} ${done?'done':''}" href="${nodeUnlocked?'#/ch/lesson/'+w.id+'/'+l.id:'#'}">
...
function renderAchievements(){
  const body = `<a href="#/ch/" style="font-size:13px;color:var(--muted)">← Home</a>
...
function renderLesson(worldId, lessonId){
  ...
  if(!w||!lesson){ location.hash='#/ch/'; return; }
    ...
    return `<div style="border-radius:32px;padding:2px;background:${grad}">
      <div style="border-radius:30px;background:#0d0818cc;backdrop-filter:blur(14px);padding:20px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
          <a href="#/ch/" style="border:2px solid var(--border);border-radius:12px;padding:6px 12px;font-weight:800">✕</a>
    ...
    if(stage==='done'){
      ...
      app.innerHTML = `<main>
        <div class="card" style="text-align:center;background:${grad};color:#fff">
          ...
          <div style="margin-top:20px"><a class="btn" style="background:#fff;color:#1a1030" href="#/ch/">Back to Progress Map</a></div>
        </div>
      </main>`;
      return;
    }
  ...
  function resolve(correct){
    ...
    if(correct){ correctCount++; combo++; bestCombo=Math.max(bestCombo,combo); }
      ...
      const q = questions[qIdx];
      if(typeof atlasRecordMistake === 'function') atlasRecordMistake('ch', w.name||'general', q ? {prompt:q.prompt, type:q.type} : null);
    }
    draw();
  }
...
window.SubjectApps['ch'] = { init: chInit };
