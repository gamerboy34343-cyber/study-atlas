...
const ssClassId = localStorage.getItem('atlas-class') || '8';
const CU_EN = (ssClassId!=='8' && window['SOCIETY_CURRICULUM_'+ssClassId]) ? window['SOCIETY_CURRICULUM_'+ssClassId] : window.SOCIETY_CURRICULUM;
const CU_HI = (ssClassId!=='8' && window['SOCIETY_CURRICULUM_'+ssClassId+'_HI']) ? window['SOCIETY_CURRICULUM_'+ssClassId+'_HI'] : window.SOCIETY_CURRICULUM_HI;
...
const ACHIEVEMENTS = [
  ...
  {id:'society-genius', title:'Society Genius', icon:'🧠', desc:'Earn 1000 XP.'},
  {id:'explorer-badge', title:'Explorer', icon:'🗺️', desc:'Visit lessons across 5 worlds.'},
  {id:'market-guardian', title:'Guardian of the Market', icon:'⚖️', desc:'Defeat the final boss.'},
];
...
/* ============================== STATE ============================== */
const STORE_KEY = 'understanding-society-save-v1' + (ssClassId!=='8' ? '-c'+ssClassId : '');
...
const key = (w,l)=>`${w}:${l}`;
function activeUnreviewedWorldSs(){
  for(const w of CU){
    const started = w.lessons.some(l=>!!STATE.completed[key(w.id,l.id)]);
    const reviewed = typeof atlasModulePassed !== 'function' || atlasModulePassed('ss', String(w.id));
    if(started && !reviewed) return w;
  }
  return null;
}
function isWorldUnlocked(worldId){
  const active = activeUnreviewedWorldSs();
  return !active || active.id===worldId;
}
function worldNeedsReviewSs(worldId){
  const active = activeUnreviewedWorldSs();
  return !!active && active.id!==worldId;
}
window.__openWorldReviewSs = ()=>{
  const active = activeUnreviewedWorldSs();
...
function completeNode(e){
  ...
  if(xp>=1000) grantBadge('society-genius');
  ...
  if(e.worldId==='pricepuzzle9' && e.lessonId==='pp-boss') grantBadge('market-guardian');
...
if(window.__ssHashListener) window.removeEventListener('hashchange', window.__ssHashListener);
window.__ssHashListener = function(){ if(!location.hash.startsWith('#/ss')) return; render(); };
window.addEventListener('hashchange', window.__ssHashListener);
function render(){
  window.scrollTo(0,0);
  const hash = location.hash.replace(/^#\/ss\/?/,'');
...
function Topbar(){
  ...
  return `<div class="topbar">
    ...
    <a class="stat-chip" href="#/ss/achievements">🏅 ${STATE.badges.length}/${ACHIEVEMENTS.length}</a>
    <a class="stat-chip" href="#/ss/shop">🛍 Shop</a>
  </div>`;
}
...
function renderHome(){
  ...
  const body = `
    <div class="card" style="text-align:center">
      ...
      <h1 class="font-display" style="font-size:30px;margin:8px 0 4px">Understanding Society</h1>
      <p style="color:var(--muted);max-width:520px;margin:0 auto">Journey through geography, history, civics and economics as one connected story — from the shaping of the Earth to the ideas that shape society today.</p>
      <div style="margin-top:18px">
        ${curTarget?`<a class="btn btn-primary" href="#/ss/lesson/${curTarget.w.id}/${curTarget.l.id}">${doneLessons?'Continue':'Start'} the adventure →</a>`:`<div class="btn btn-primary" style="display:inline-block">🏆 All missions recovered!</div>`}
      </div>
    </div>
    ...
    <div>
      ${CU.map(w=>{
        ...
        const needsReview = worldNeedsReviewSs(w.id);
        const prog = worldProgress(w.id);
        const clickAttr = needsReview ? `onclick='__openWorldReviewSs()'` : '';
        return `<div class="world-card ${unlocked?'':(needsReview?'needs-review':'locked')}" ${clickAttr} style="background:linear-gradient(135deg,${w.palette.from},${w.palette.to});${needsReview?'cursor:pointer':''}">
          ...
          ${unlocked?`<div class="node-grid">${w.lessons.map(l=>{
            ...
            return `<a class="node ${nodeUnlocked?'':'locked'} ${done?'done':''}" href="${nodeUnlocked?'#/ss/lesson/'+w.id+'/'+l.id:'#'}">
...
function renderAchievements(){
  const body = `<a href="#/ss/" style="font-size:13px;color:var(--muted)">← Home</a>
...
function renderLesson(worldId, lessonId){
  ...
  if(!w||!lesson){ location.hash='#/ss/'; return; }
    ...
    return `<div style="border-radius:32px;padding:2px;background:${grad}">
      <div style="border-radius:30px;background:#0d0818cc;backdrop-filter:blur(14px);padding:20px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
          <a href="#/ss/" style="border:2px solid var(--border);border-radius:12px;padding:6px 12px;font-weight:800">✕</a>
    ...
    if(stage==='done'){
      ...
      app.innerHTML = `<main>
        <div class="card" style="text-align:center;background:${grad};color:#fff">
          ...
          <div style="margin-top:20px"><a class="btn" style="background:#fff;color:#1a1030" href="#/ss/">Back to Progress Map</a></div>
        </div>
      </main>`;
      return;
    }
  ...
  function resolve(correct){
    ...
    if(correct){ correctCount++; combo++; bestCombo=Math.max(bestCombo,combo); }
      ...
      const q = questions[qIdx];
      if(typeof atlasRecordMistake === 'function') atlasRecordMistake('ss', w.name||'general', q ? {prompt:q.prompt, type:q.type} : null);
    }
    draw();
  }
...
window.SubjectApps['ss'] = { init: ssInit };
